...
def inputInfo():
    ...
    groupName = input("Enter space group "
    ...
    if (spaceGroup.xtalSystem.lower() == "triclinic"):
        ...
        angle[0] = angle[1] = angle[2] = 90
        length[0] = eval(input("Enter cell parameter (a): "))
    ...
    #   if necessary)
    wavelength = eval(input("Enter the wavelength: "))
    sMin, sMax = eval(input("Enter the sin(theta)/lambda interval: "))
    adjusted = False
    if (sMin < 0):
        sMin = 0
        adjusted = True
...
def calcStructFact(refList, atomList, spaceGroup, wavelength):
    ...
    calc.restype = None
    structFacts = [c_float() for i in range(refList.numReflections)]
    reflections = refList[:]
...
#                        np.array([np.array(so.rot, dtype=float) for so in spaceGroup.symmetryOps])
    for i in range(symmetry.numSymOps):
        # Basis 1 (from SARAh)
